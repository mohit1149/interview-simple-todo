...
class SimpleTodos extends Component {
  ...
  toggleComplete = id => {
    this.setState(prevState => {
      const updatedTodoList = prevState.todosList.map(todo =>
        todo.id === id ? {...todo, completed: !todo.completed} : todo,
      )
      return {todosList: updatedTodoList}
    })
  }

  editFunctionUpdate = (id, updatedTitle) => {
    this.setState(prevState => {
      const updatedTodoList = prevState.todosList.map(todo =>
          ...
          <ul className="todos-list">
            {todosList.map(todo => (
              <TodoItem
                ...
                editFunction={this.editFunctionUpdate}
              />
            ))}
          </ul>
        </div>
